angular
  ...
  .component('customSearchBookmarkFilter', {
    ...
    template: `<button ng-repeat="item in items" aria-label="{{ translate(item.description) }}" ng-click="goToUrl(translate(item.action))" class="button-with-icon search-bookmark-filter zero-margin md-button {{item.cssClasses}}" type="button">
